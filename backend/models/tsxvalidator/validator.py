...
if __name__ == "__main__":
    ...
    # Проверка окружения
    issues = validator.check_environment()
    if issues:
        logger.warning("Обнаружены проблемы с окружением:")
        for issue in issues:
            logger.warning(f" - {issue}")
    else:
        logger.info("Окружение настроено корректно")

    # Пример валидации TSX кода
    tsx_code = """
    // Import necessary components
import React, { useState } from 'react';
import { Grid, Box, Card, Tabs, Input, Button } from '@nlmk/ds-2.0';
...
const Interface = () => {
  ...
  return (
    <Grid borderRadius="var(--4-border)" st={{ width: '100%', padding: 0 }}>
      <Grid.Row borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
            ...
            <Grid.Row borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
              <Box px="var(--8-space)" py="var(--16-space)" borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
                <Card
                  ...
                  description="Кликабельные карточки с изображением результата прошлых генераций"
                  isStable
      ...
      <Grid.Row borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
        <Box px="var(--8-space)" py="var(--16-space)" borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
          <Tabs>
            <Tabs.Tab label="Рендер" active={activeTab === 0} onClick={() => setActiveTab(0)} />
            <Tabs.Tab label="Код" active={activeTab === 1} onClick={() => setActiveTab(1)} />
          </Tabs>
        </Box>
      </Grid.Row>
      <Grid.Row borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
        <Box px="var(--8-space)" py="var(--16-space)" borderRadius="var(--4-border)" background="var(--primary-blue-400)" st={{ flex: '1' }}>
          <Input multiline label="Введите запрос" helperText="Поле для ввода запроса пользователя" />
...
export default Interface;
    ...
    result = validator.validate_tsx(tsx_code)
